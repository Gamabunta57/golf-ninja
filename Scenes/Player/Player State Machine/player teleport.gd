...
func process_physics(delta: float) -> State:
	# Force velocity to zero to prevent move_and_slide drifting
	...
	if inputs.get_x_input() != 0 and sign(inputs.get_x_input()) != Global.direction:
		return fall_state
		
	# Skip move_and_slide() while teleporting/sticking to keep position 100% locked
